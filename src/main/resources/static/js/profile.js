...
function toggleSubscribe(toUserId,obj) {
	if ($(obj).text() === "구독취소") {
		...
		$.ajax({
			type:"delete",
			url:"http://localhost:8080/api/subscribe/"+toUserId,
			dataType:"json"
		}).done(res=>{
			$(obj).text("구독하기");
...
function subscribeInfoModalOpen(pageUserId) {
	...
	$.ajax({
		...
		//res를 콘솔 출력해보면 원하는 리스트가 data속성안에 있다.
		res.data.forEach((u)=>{
...
function profileImageUpload(pageUserId,principalId) {
	...
	$("#userProfileImageInput").on("change", (e) => {
		...
		$.ajax({
			...
			processData:false,//필수: contentType을 false를 줬을때 QueryString 자동설정되므로 그것도 해제,
